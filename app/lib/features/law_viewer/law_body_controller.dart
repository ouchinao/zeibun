...
class LawBodyController
    extends AutoDisposeFamilyAsyncNotifier<LawText, String> {
  ...
  @override
  Future<LawText> build(String arg) async =>
      LawText.from(await ref.read(lawRepositoryProvider).openLaw(arg));
  ...
  Future<void> _reload(
    ...
    state = const AsyncLoading<LawText>().copyWithPrevious(state);
    state = await AsyncValue.guard(() async => LawText.from(await ref
        .read(lawRepositoryProvider)
        .openLaw(arg, includeAmendSuppl: includeAmendSuppl, force: force)));
  }
}
