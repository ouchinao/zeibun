...
法令のツリー・パーサ・スコープ・合成法令は Phase 1 で [`../packages/zeibun_core`](../packages/zeibun_core) に移した。
...
| `bin/spike.dart` | CLI（下記）。合成法令は `package:zeibun_core/testing.dart` の `SyntheticLaw` |
