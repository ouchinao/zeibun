...
名前の zeibun は「税文」、税の条文から。アイコンは条文のカードに「税」。
...
| # | 項目 | やること | いまの状態 | 根拠・注意 |
|---|---|---|---|---|
| R1 | 名前と識別子 | 表示名を「zeibun 税法検索」（ホームの題と同じ）に揃える。`applicationId` / Bundle ID を **`io.github.ouchinao.zeibun`** に変える（決定、v0.9）。ドメインは取らない | **完了**: ID を両 OS とも `io.github.ouchinao.zeibun` に変更。表示名は両 OS とも `zeibun`（ストアの題名は掲載時に別途付ける） | ID は**公開後に変更できない**。`zeibun.jp` を保有しておらず取る予定もないので、他人のドメイン名を名乗らない形にする。ストアはドメインの所有を確認しないが、逆引き形式は所有を前提にした慣習。名称・アイコンに政府機関のロゴを使わない（§11 S） |
| R2 | アイコンと起動画面 | 1024px の原画を 1 枚作り、`flutter_launcher_icons` で Android の adaptive icon と iOS の全サイズを生成。起動画面は既定の白のまま | **仮アイコンで完了**: 濃紺地（#1F3A5F）に生成りのカード（#F6F1E7）、本文の行 3 本と紺の「税」。名前の由来「税の条文」を絵にした。原画は `app/assets/icon/`、`dart run flutter_launcher_icons` で iOS 全サイズと Android の adaptive icon を生成。公開後に原画を差し替えて再生成すればよい（アイコンは識別子と違って後から変えられる） | ストア審査で既定アイコンは通らない。起動画面に凝らないのは、初回描画までが 1 秒未満で凝る意味がないため |
| R3 | 署名とバージョン | iOS: Apple Developer Program に登録し Xcode の自動署名。ビルドとアップロードは手元の Mac（Xcode）から。TestFlight で自分の端末に配って確認する（テスター人数の要件は無い）。`pubspec.yaml` の `version` を `x.y.z+ビルド番号` で運用し、リリースごとに `v x.y.z` タグを打つ。Android のアップロード鍵と Play App Signing は Android 公開時に | 手順書は `docs/release.md` に作成済み。`version: 0.1.0+1`。Apple Developer Program の登録と Xcode の署名設定は未。Android の release ビルドは **debug 鍵で署名**（`build.gradle.kts` の TODO） | ビルド番号は単調増加が必須。手順は `docs/release.md` に書く（バージョン更新 → タグ → `flutter build ipa` → アップロード）。Android の署名を後にしても、ビルド番号は両 OS で同じ列を使うので運用は変わらない |
...
| R7 | 配布地域・言語・端末 | 日本のみ、日本語のみ。v1 は iOS のみで、iPhone と iPad（現状 `TARGETED_DEVICE_FAMILY = 1,2`）の両方を対象にする（決定、v0.9）。Android は後日 | 端末ファミリは両方。`ITSAppUsesNonExemptEncryption = false` は Info.plist に設定済み。地域設定は App Store Connect で未 | 日本のみにするのは、内容が日本の法令であることに加え、EEA 向けの同意管理（UMP）や各国の消費税手続きを避けるため。iPad を残すのは、条文を横に広く読む用途があり、対応の追加コストがスクリーンショットだけのため。`ITSAppUsesNonExemptEncryption = false` を Info.plist に置く（HTTPS のみで輸出規制の申告が不要） |
| R8 | ストア掲載物 | スクリーンショット（iPhone 6.9″ / 6.5″、iPad 13″。Android 端末・7″・10″ は Android 公開時に）、説明文、カテゴリ（仕事効率化）、年齢区分のアンケート、サポート URL（GitHub の Issues）、R5 のポリシー URL。説明文の冒頭に「デジタル庁・e-Gov の公式アプリではない」を入れる | 名前・サブタイトル・説明文・キーワード・カテゴリの下書きは `store/description.md`。スクリーンショットは未 | 出典明記は PDL1.0 の条件（§10）。`store/` 以下に掲載物を置き、文言はアプリ内の `app_notices.dart` と揃える |
| R9 | 審査で問われそうな点 | App Store 4.2（最小限の機能）: 公開データを表示するだけと見なされないよう、審査メモにオフライン閲覧・横断全文検索・ブックマーク・改正検知を書く。Google Play（後日）: 2023-11 以降に作った個人アカウントは製品版の前に **14 日間のクローズドテスト**が必要で、テスターを人数ぶん集める必要がある（人数の要件は Play Console の最新の案内で確認） | App Store 向けの審査メモの下書きは `store/review-notes.md` | Android を後にした理由がこのテスターと実機。Play のクローズドテストは着手から公開まで最短でも 2〜3 週間かかる |
| R10 | 公開前に直す積み残し | #13 保存中の画面消灯の抑止（`wakelock_plus`）。#14 は空き容量の事前確認ではなく、書き込み失敗（SQLITE_FULL）を `PrefetchOutcome` の失敗理由として分けて出すだけにする | **完了**: `PrefetchService` が実行中だけ `keepScreenOn`（`wakelock_plus`）を立て、終了・中断・失敗のいずれでも戻す。SQLITE_FULL は `PrefetchOutcome.storageFull` として「空き容量を増やして再実行」を案内 | 全法令の保存は数分〜数十分かかり、画面が消えると OS に止められる。事前の空き容量取得はプラグインの保守が薄く、失敗理由を分ける方が確実 |
| R11 | CI | `flutter build apk --debug` を CI に足し、Android のビルド破損を PR で検知する。Android を後回しにするからこそ要る（手元で組まない期間に腐るのを防ぐ）。iOS は macOS ランナーの費用がかかるので手元でビルドする | **完了**: `flutter-android` ジョブで `flutter build apk --debug --target-platform android-arm64`。iOS は手元でビルド。SQLite は `package:sqlite3` 3.x が hooks でビルド時にコンパイルする構成（`sqlite3_flutter_libs` は EOL） | Gradle・AGP・Kotlin の更新（Dependabot は見ない）で Android だけ壊れることがある。Play の要件も確認する: `targetSdk` は最新から 1 年以内、ネイティブライブラリは **16KB ページサイズ対応**（hooks でビルドした SQLite の `.so` を `check_elf_alignment` で確認）。FTS5 が実機のビルドで有効なことは R4 で確かめる |
| R12 | 公開前の通し確認 | 実機で 初回起動 → 免責 → 同期 → 所得税法を開く → 機内モードで再起動して読める → 横断全文検索 → 全法令保存を中断・再開。§11 の「MVP」行を再点検。設定画面にアプリのバージョンを出す（問い合わせ時に聞くため）。リリースノートを書く | バージョン表示は設定画面に追加済み。通し確認は実機で未 | §12 の「結合（手動）」を公開前の受け入れ試験として実施する |
| R13 | バックアップからの除外 | DB ファイルを OS のバックアップ対象から外す（iOS `NSURLIsExcludedFromBackupKey`、Android `dataExtractionRules`）。除外しても閲覧履歴・ブックマークは端末内に残る（機種変更では引き継がれない旨をポリシーに書く） | **完了**: `drift_flutter` の既定（Documents、バックアップ対象）から Application Support 下の `db/` に移し、iOS は `AppDelegate` のチャネルでディレクトリごと除外、Android は `dataExtractionRules` / `fullBackupContent` で除外 | 全法令を保存すると数百 MB。iCloud の 5GB を食い、App Store は再取得できるデータのバックアップを審査で問う。Android の自動バックアップは 25MB 上限で黙って失敗するだけだが、明示しておく（§11 I） |
