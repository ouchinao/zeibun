...
- 選択肢:
  - **A. 既定で除く、法令ごとに全文へ切替**（現行案）。初回表示が軽い。経過措置を読むときにワンタップと再取得（所得税法で +450KB、数秒）が要る
  - **B. 主要法令だけ全文を既定にする**。所得税法・法人税法・租税特別措置法・地方税法とその政省令の約 12 件は常に全文（wire 合計約 6MB、パースは全文用のストリーム実装が前提）。他は A
  - **C. 常に全文**。所得税法で 16MB・要素 20 万をストリーム処理する前提。中位 Android の体感が数秒〜十数秒になる
- 附則の表示は、どの案でも「改正法令番号ごとに折りたたみ、新しい順」にする（1,000 件超を平置きしない）
...
中継サーバや Web 固有のライブラリ制限で「できない」ものは無い。増えるのはホスティングの選定、Worker、フォント同梱、Safari の永続性の 4 点で、いずれもモバイル版の設計を変えずに後から足せる。
...
1. **対象範囲**: 「国税」分類 ＋ 地方税法まわり ＋ 明示 12 件でよいか。関税関係を含めてよいか（廃止・失効法令は「参考」として載せることで決定済み）
2. **Web**: MVP に含めるか、モバイル MVP の後にするか（§9 の制約を参照）
3. **改正附則の既定**: 本文は既定で改正法令の附則を除いて取得し、法令ごとに「改正附則も読む」で全文を取り直す設計にした。税務実務では経過措置（改正附則）を参照する場面があるので、主要法令だけ既定を全文にする案もある。どちらにするか（判断材料は §4.5）
