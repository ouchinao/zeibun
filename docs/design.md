...
- 状態: ドラフト v0.8（2026-09-23）— Phase 1・2 実装済み。公開準備（Phase R）を Phase 3 の前に追加
...
「公開」は **App Store と Google Play で、日本向け・日本語のみ**。Web 版は出さない（v0.5 の決定）。
...
| # | 項目 | やること | いまの状態 | 根拠・注意 |
|---|---|---|---|---|
| R1 | 名前と識別子 | 表示名を「zeibun 税法検索」（ホームの題と同じ）に揃える。`applicationId` / Bundle ID を確定する | Android の表示名 `zeibun`、iOS `Zeibun`。ID は `jp.zeibun.zeibun` | ID は**公開後に変更できない**。逆引きドメインの `zeibun.jp` を保有していないなら、保有するドメインか `io.github.ouchinao.zeibun` に今のうちに変える。名称・アイコンに政府機関のロゴを使わない（§11 S） |
...
| R3 | 署名とバージョン | Android: アップロード鍵を作り Play App Signing に登録。`key.properties` は git 管理外。iOS: Apple Developer Program に登録し Xcode の自動署名。`pubspec.yaml` の `version` を `x.y.z+ビルド番号` で運用し、リリースごとに `v x.y.z` タグを打つ | `version: 0.1.0+1`。Android の release ビルドは **debug 鍵で署名**（`build.gradle.kts` の TODO） | ビルド番号は両ストアとも単調増加が必須。手順は `docs/release.md` に書く（バージョン更新 → タグ → `flutter build appbundle` / `flutter build ipa` → アップロード） |
| R4 | 実機計測 | 中位 Android（Snapdragon 6xx 級・RAM 4GB）と手持ちの iPhone で、所得税法（既定）・地方税法・租税特別措置法（全文）の初回表示時間とメモリ、全法令の保存の所要時間と容量を測る | Phase 0 の卓上計測のみ。Phase 1 で「最初にやる」としたまま未実施 | 合格線: 既定取得の初回表示 3 秒以内、全文取得で OOM しない、全法令保存が Wi-Fi で 15 分以内。超えるなら §10 の「巨大法令の体感」を見直す |
| R5 | プライバシーポリシーと申告 | `docs/privacy.md` を書き GitHub Pages で URL を持つ。内容: アカウント無し、個人情報の収集無し、通信先は `laws.e-gov.go.jp` のみ、閲覧履歴・ブックマークは端末内のみ。App Store の App Privacy は「データを収集しない」、Play のデータセーフティも同じ内容で申告 | 無し | 両ストアとも収集ゼロでもポリシー URL は必須。§11 I のとおり分析・クラッシュ SDK は入れないので申告が単純になる。広告を入れる場合はこの項目が大きく変わる（R6）。あわせて e-Gov 法令API の利用規約（出典表示・負荷・保証なし）を公開前にもう一度読む |
| R6 | 収益化の方式 | §14-4 で決める。案 A: 買い切り（¥500〜¥1,000）。案 B: 無料＋広告。案 C: 無料＋アプリ内課金（広告除去・全法令保存）。**推奨は A** | 未決 | B は広告 SDK が入り、iOS の ATT 許諾、Play の広告 ID 申告、ポリシーの第三者提供の記載が要る。§11 I の「第三者に何も送らない」設計と食い違い、UA 以外の識別子を送ることになる。A は決済も配信もストアが担い、アプリ側の変更が無い。税務の実務者は道具に対価を払う層で、広告収入より単価が読める。ただし有料にすると R14 の開発者情報の公開が付いてくる |
| R7 | 配布地域・言語・端末 | 日本のみ、日本語のみ。iPhone と iPad（現状 `TARGETED_DEVICE_FAMILY = 1,2`）の両方を対象にする | 端末ファミリは両方。地域設定は未着手 | 日本のみにするのは、内容が日本の法令であることに加え、EEA 向けの同意管理（UMP）や各国の消費税手続きを避けるため。iPad を残すのは、条文を横に広く読む用途があり、対応の追加コストがスクリーンショットだけのため。`ITSAppUsesNonExemptEncryption = false` を Info.plist に置く（HTTPS のみで輸出規制の申告が不要） |
| R8 | ストア掲載物 | スクリーンショット（iPhone 6.9″ / 6.5″、iPad 13″、Android 端末・7″・10″）、説明文、カテゴリ（仕事効率化）、年齢区分のアンケート、サポート URL（GitHub の Issues）、R5 のポリシー URL。説明文の冒頭に「デジタル庁・e-Gov の公式アプリではない」を入れる | 無し | 出典明記は PDL1.0 の条件（§10）。`store/` 以下に掲載物を置き、文言はアプリ内の `app_notices.dart` と揃える |
| R9 | 審査で問われそうな点 | App Store 4.2（最小限の機能）: 公開データを表示するだけと見なされないよう、審査メモにオフライン閲覧・横断全文検索・ブックマーク・改正検知を書く。Google Play: 2023-11 以降に作った個人アカウントは製品版の前に **14 日間のクローズドテスト**が必要（テスター人数の要件は Play Console の最新の案内で確認） | — | Play のクローズドテストは着手から公開まで最短でも 2〜3 週間かかる。Phase 3 の途中で始めても遅くない |
...
| R14 | 開発者情報の公開 | 有料にする場合、Google Play は販売者の**住所の公開**を求め、日本向けには特定商取引法に基づく表記（氏名・住所・電話）を求められる。App Store も日本のストアで販売者情報の表示要件がある。個人の住所を出したくなければ、バーチャルオフィスの住所を使うか、無料（R6 案 B）にするか、法人で出す | — | 個人開発者が見落としやすい。R6 の決定と一体で決める。Play は開発者アカウントの本人確認（身分証）も必要で、日数がかかる |

成果物: `docs/privacy.md`、`docs/release.md`（手順書）、`store/`（掲載物）、アイコン原画、署名の設定（鍵は git 管理外）、CI の Android ビルド、#13 / #14 の対応、実機計測の結果（この表の R4 に追記）。

順番: R1・R6・R14 の決定 → R3（開発者登録と本人確認は待ち時間があるので最初に）→ R2・R5・R7・R13 → R10・R11 → R4 → R8・R9 → R12。
...
4. **収益化の方式**（Phase R の R6）: 買い切り / 無料＋広告 / 無料＋アプリ内課金。推奨は買い切り。広告にするなら R5 の申告と §11 I の設計を見直す
5. **識別子**（R1）: `jp.zeibun.zeibun` のままでよいか。`zeibun.jp` を保有していなければ、保有ドメインか `io.github.ouchinao.zeibun` に変える。公開後は変更できない
6. **対応端末**（R7）: iPad を対象に含めるか（推奨は含める。追加コストはスクリーンショットのみ）
